...
public  class EntityAction<T> extends BaseDaoAction implements IBaseEntityAction {
	...
	protected Class<T> entityClass; // Action拥有的Entity类型.
	
	protected T entity;//从ViewStack中实体
	...
	/**
	 * 初始化action对象,Struts2EntityAction{@link #execute()}调用
	 */
	private void init()
	{
		entityClass = ReflectUtil.getSuperClassGenricType(getClass());
		log.debug("执行init方法后entityClass为："+entityClass.getName());
	}
	
	/**
	 * 在新增实体的时候做一些初始化，比如用户默认密码
	 */
	...
	public String addMethod() throws Exception
	{
		...
		log.info("需要添加对象的jsondata为："+jsondata);
		this.entity = (T) JSONObject.toBean( obj, this.entityClass );
		
		if(log.isInfoEnabled())
			log.info("需要添加对象的jsondata转化成实体为："+entity);
			*/
	...
	public String deleteMethod() throws Exception
	{
		try
		{
			...
			if(log.isInfoEnabled())
			{
				log.info("请求删除的ids为："+this.ids);
				log.info("需要删除的记录有："+ids.length+"条");
	...
	public String editMethod() throws Exception
	{
		...
		{/*
			JSONObject obj = JSONObject.fromObject(this.jsondata);
			log.info("需要编辑对象的jsondata为："+jsondata);
			this.entity = (T) JSONObject.toBean( obj, this.entityClass );
			log.info("需要编辑对象的jsondata转化成实体为："+entity);
			*/
